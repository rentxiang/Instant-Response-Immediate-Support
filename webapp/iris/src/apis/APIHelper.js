...
const BASE_API_URL = "http://localhost:9000/requests";
const BASE_CALLER_API_URL = "http://localhost:9000/caller";
const BASE_USER_API_URL = "http://localhost:9000/userInfo";
...
const loginUser = async (payload) => {
  ...
  const response = await fetch("http://localhost:9000/login", requestParameters);
  const data = await response.json();
  return {
    response,
    data
...
const newUser = async (payload) => {
  ...
  fetch("http://localhost:9000/userInfo", requestParameters)
...
const updatePassword = async (payload) => {
  ...
  const path = "http://localhost:9000/userInfo/" + `${payload.id}`;
 ...
 */
 const updateUser = async (payload, token) => {
  ...
  const path = "http://localhost:9000/userInfo/" + `${payload._id}`;
...
const deleteUser = async (payload) => {
  ...
  fetch(`http://localhost:9000/userInfo/${payload._id}`, requestParameters)
    .then(response => response.json())
    .then(data => {
      alert('User Deleted');
    }).catch(error => {
...
const getAllPhysicians = async (token) => {
    const options = {
        method: "GET",
        headers: { "Content-Type": "application/json", authorization: `Bearer ${token}` }
      };
  const response = await fetch(`${BASE_USER_API_URL}?role=Physician`, options);
  const data = await response.json();
  return data;
}
