...
class PhysicianHome extends Component {
  ...
  async fetchRequestsByReceiver() {
    APIHelper.fetchRequestsByReceiver("Physician_1", "physician", this.props.user.state.accessToken).then(
      (data) => {
        console.log(data);
  ...
  getSelectedRow(ids) {
    ...
    APIHelper.fetchCallerDetails(selectedRow.caller.callerId, this.props.user.state.accessToken).then((data) => {
      ...
      <div className="firstResponderContainer">
        ...
        <div className="form-map-container">
          <div className="emergencyForm">
            <CustomizedDialogs open={this.state.openVitalsDialog} closeHandle={this.closeVitalsDialog.bind(this)} content={this.state.callerMedicalHistory} />
          </div>
        </div>
      </div>
    );
